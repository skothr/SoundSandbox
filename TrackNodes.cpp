...
AudioTrackNode::AudioTrackNode(int sample_rate, c_time track_length)
	: Node(NType::AUDIO_TRACK, "Audio Track", "Stores a buffer of audio data."),
...
void AudioTrackNode::onUpdate(double dt)
{
	//Update length
	if(midiNode)
		setLength((c_time)ceil(midiNode->getLength()*(double)sampleRate/(double)AUDIO_CHUNK_SIZE));
...
bool AudioTrackNode::pullData(PullPacket &output, NCID this_id)
{
	bool pulled = false;
...
bool AudioTrackNode::pushData(PushPacket &input, NCID this_id)
{
	bool pushed = false;
...
void AudioTrackNode::update(double dt)
{
...
MidiTrackNode::MidiTrackNode(c_time track_length)
	: Node(NType::MIDI_TRACK, "MIDI Track", "Stores a buffer of MIDI notes."), TrackNode(track_length), midiData(), initial_mods()
{
	initNode();
}
...
void MidiTrackNode::onUpdate(double dt)
{
	/*
	if(pRecordCursor)
	{
...
bool MidiTrackNode::pullData(PullPacket &output, NCID this_id)
{
	bool pulled = false;
...
bool MidiTrackNode::pushData(PushPacket &input, NCID this_id)
{
	bool pushed = false;
...
ModTrackNode::ModTrackNode(c_time track_length)
	: Node(NType::INVALID, "Null Mod Track Node", "Unspecified"), TrackNode(track_length), ModNode(), points()
{
	reset();
}
...
AudioModTrackNode::AudioModTrackNode(AudioModFunction mod_function, c_time track_length)
	: Node(NType::AUDIO_MOD_TRACK, "Audio-Mod Track", "Modifies audio data over time."), ModTrackNode(track_length), modify(mod_function)
{
	initNode();
}
...
//TODO: Check this
bool AudioModTrackNode::pullData(PullPacket &output, NCID this_id)
{
	bool pulled;
...
bool AudioModTrackNode::pushData(PushPacket &input, NCID this_id)
{
	bool pushed;
...
MidiModTrackNode::MidiModTrackNode(SampleModFunction mod_function, c_time track_length)
	: Node(NType::MIDI_MOD_TRACK, "MIDI-Mod Track", "Modifies MIDI data over time."), ModTrackNode(track_length), modify(mod_function)
{
	initNode();
}

MidiModTrackNode::MidiModTrackNode(const MidiModTrackNDesc &mmtn_desc)
	: Node(*(NDesc*)&mmtn_desc), ModTrackNode(),
...
bool MidiModTrackNode::pullData(PullPacket &output, NCID this_id)
{
	bool pulled;
...
bool MidiModTrackNode::pushData(PushPacket &input, NCID this_id)
{
	bool pushed;
